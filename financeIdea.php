<?php
    ...
    if($response == "ok") {
        ...
        $text = mysqli_real_escape_string("Complimenti! Hai finanziato l'idea {$idea['Idea']['nome']}! Mettiti in contatto con l'ideatore tramite l'email: {$userIdea['email']}");
        ...
        $text = mysqli_real_escape_string("Complimenti! Hai ottenuto un finanziamento per l'idea {$idea['Idea']['nome']}! Mettiti in contatto con il finanziatore tramite l'email: {$userFinancier['User']['email']}");
        insertNotice($mail_destinatario, $idIdea, $text, "Financier");
